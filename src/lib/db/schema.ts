...
// Engineering backlog filed via the floating fix button on every dashboard
// page. `url` + `pageHtml` snapshot the page Pat was on at the moment he
// filed, so the /fixthings slash command has full context to work the fix
// without needing to open the same surface himself. Status flow:
// open → in_progress → done | needs_decision | wont_fix.
export const fixes = pgTable(
  "fixes",
  {
    ...
    filedByUserId: uuid("filed_by_user_id")
      .notNull()
      .references(() => users.id, { onDelete: "restrict" }),
    filedByEmail: text("filed_by_email").notNull(),
    status: text("status", {
      enum: ["open", "in_progress", "needs_decision", "done", "wont_fix"],
    })
      .notNull()
      .default("open"),
    category: text("category", {
      enum: ["ui", "data_wrong", "automation", "idea", "other"],
    }),
  ...
  (t) => [
    index("idx_fixes_status").on(t.status),
    index("idx_fixes_created_at").on(t.createdAt),
  ]
);
