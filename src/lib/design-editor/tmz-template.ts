/**
 * The "TMZ" preset — one square image, modelled on the format's best posts
 * (Sept 2026; the published ones are 4:5 or 6:5, but everything the design
 * editor makes is a square — Pat's rule): a still of the founder over the
 * top ~55%, a black band
...
export function buildTmzPage(): DesignPage {
  ...
  const elements: DesignElement[] = [
    ...
    text("Headline", { x: 40, y: photoH + 24, w: W - 80, h: H - photoH - 48 }, applyHighlights("My website that converts bank statements makes over $40,000 every month", [{ phrase: "$40,000", color: "green" }]), baseText({
      fontId: "anton", sizePx: 84, lineHeight: 1.06, align: "center", valign: "middle", uppercase: true, autoFit: true, minSizePx: 44,
...
export function buildTmzTemplate(): DesignDoc {
  return { version: 1, canvas: { ...IG_SQUARE }, template: TMZ_TEMPLATE, pages: [buildTmzPage()] };
}
