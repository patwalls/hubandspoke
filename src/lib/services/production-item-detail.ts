...
/**
 * Everything the content-detail page needs, in one call — extracted verbatim
 * from the GET /api/production-items/[id] route (2026-08-10) so it can be:
 *   1. SSR'd by the detail page (initial data in the HTML — no client
 *      shell->hydrate->fetch waterfall, which cost ~2-3s felt),
 *   2. executed by /api/warm at boot (importing the route module measured as
 *      a no-op for warming; EXECUTING the service warms the module graph,
 *      pool, and query plans for real),
 *   3. still served by the API route for client-side refetches after
 *      mutations.
 * Returns null when the item doesn't exist. Throws on real errors (callers
 * decide the envelope).
 */
export async function getProductionItemDetail(id: string) {
...
export type ProductionItemDetailPayload = NonNullable<
  Awaited<ReturnType<typeof getProductionItemDetail>>
>;
