...
- **What it renders:** each page's `DesignPage` → satori element tree (absolutely positioned single-line text nodes from the shared layout engine — the browser stage draws the identical markup, so preview == export) → SVG → **resvg** → 1080×1080 PNG → S3 (`<prefix>/<itemId>/<uuid>-design-page-N.png`) → `installDesignMedia`: pages take `production_item_media` indexes 0…n-1 with `source_url = design-editor://render/<renderId>`, other media renumbered after them, a previous design render replaced, manual uploads kept; legacy cover columns mirrored; `media_added`/`media_removed` under `source: { kind: "tool", tool: "design-editor" }`. No browser, no Chrome: satori (Vercel's HTML/CSS→SVG) + `@resvg/resvg-js` prebuilt native binary. ~1–2 s per page.
