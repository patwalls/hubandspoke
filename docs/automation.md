...
USER / API ENTRY POINTS
  ...
  POST /api/production-items/[id]/cross-post              → descript-derivative-create mode=cross-post (composition copy; cross-aspect path: gated on hasDescriptableMedia, transcripts on source+pillar required)
  POST /api/production-items/[id]/cross-post              → descript-derivative-create mode=repost      (composition copy; SAME-ASPECT shortcut — IG Reel↔TikTok↔Story; gated on checkRepostReadiness, no transcripts, source's own media is reused)
...
- **Outputs:** a new `contentDrafts` row via the demote-then-insert tx (current row → `is_current=false`, new row → `is_current=true`, same `production_item_id`, version+1). `generatedBy = "draft-algo:v1.7:claude-opus-4-7:v12"` — the prefix tags the algorithm version, the suffix carries the underlying agent model+prompt version (`PROMPT_VERSION` bumped to 12 for the CTA BASELINE TEMPLATE — see CTA reply rule below). `modelUsage` populated. **V1.1 media write still active:** if the agent's `media_action` is `attach_pillar_full_video` or `attach_pillar_poster` AND the item has zero existing `production_item_media` rows AND the action is fulfillable (pillar has the asset, platform rule accepts the kind), the algorithm inserts a media row pointing at the pillar's S3 key (byte-cost zero — same bucket reuse) inside the same tx, and mirrors `mediaS3Key/Bucket/ContentType/posterS3Key` on `productionItems` to keep legacy single-media columns consistent.
- **Downstream:** none.
- **Rules:**
...
- **Trigger:** enqueued by `POST /api/production-items/[id]/cross-post` and `POST /api/production-items/[id]/repost` after the new derivative row is inserted. The cross-post route picks `mode=cross-post` OR `mode=repost` per-call based on `aspectMatchesTarget(source.postType, targetPostType)` — same-aspect cross-posts (IG Reel ↔ TikTok ↔ Story, all 9:16) are treated like reposts because no re-aspecting is needed; cross-aspect cross-posts (e.g. Reel → square IG Post) keep the legacy pillar-anchored flow. The repost route always sends `mode=repost`. Each route hard-gates upfront — `checkCrossPostReadiness(source, pillar)` for the cross-aspect path, `checkRepostReadiness(source)` for the same-aspect / repost path — so reaching this task means a viable path to a composition exists. **Manual escape:** cross-post accepts `{ manual: true }` which bypasses the readiness gate AND skips this enqueue entirely; the row is created empty for the operator to attach media themselves. Surfaced in `cross-post-triage-dialog.tsx` as the "I'll do it manually" button on the inline gate-failure banner.
