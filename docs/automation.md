...
- **Rules:**
  ...
  - **Descript branch (V1.7, 2026-05-14; cold-import added 2026-05-15):** after the caption-write tx commits, the algorithm checks `formats.is_clip_descript_format`. When true, it runs `runDescriptStepForDerivative` (in `src/lib/services/draft-algorithm/descript-step.ts`) which extracts the format Skill's `### Descript Clip & Pack Info` section verbatim — no `{{hook}}`/`{{startTimestamp}}` substitution because there's no clip-idea row backing this; the Skill is self-contained (e.g. "clip out the intro and only the intro"). Picks one of two paths by pillar state:
