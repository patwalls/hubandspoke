...
AUTO-CHAINS (one task enqueues another)
  ...
  transcribe-whisper ── new Starter Story yt_long only ─→ generate-clip-ideas (per clippable format; gates in clip-ideas-auto.ts)
...
- **Routing model (2026-08-10):** routing is now account-based, not format-based. The sweep looks up which target formats are mapped to the pillar's `account_id` via the `format_trigger_sources` table, then reads each target format's `view_threshold` from the `formats` row. This replaces the old `pillar.format → parent/child format tree` traversal — `production_items.format` is no longer required on pillar items for triggering to fire. `format_trigger_sources` rows are seeded manually (or via backfill script); there is no UI yet.
- **`format_trigger_sources` table:** `(format_id, source_account_id)` pairs — "pillars from this account trigger this target format." One unique row per (format, account). `viewThreshold` always reads live from `formats.view_threshold`; it is not stored on the routing table.
- **Rules:**
...
- **Archive-step failures (2026-08-11 fix):** the download+archive step runs *after* Descript has already reported success, so it sits outside both the deadline check (which only runs on the `job_state !== "stopped"` branch) and every other error-stamping branch. It used to throw straight through without touching the row, leaving `job_id set / published_at null / error null` — the exact tuple every consumer reads as "in flight". Symptom: the pill spins "Rendering MP4…" forever with **no Retry button**, and the ops sweep's stuck-render detector (which requires `descript_publish_error IS NULL`) reports the item as healthy. Prod item `3b015853` sat like this for ~24h across 12 silent queue retries. The archive call is now wrapped: any failure stamps `descript_publish_error` first, then
  - **oversize** (`MediaTooLargeError`, > `MAX_MEDIA_BYTES` = 200 MB in `src/lib/services/enrichment/shared.ts`) → **returns without throwing**. The render is finished and the file is that big, so retrying re-downloads identical bytes 24 more times and ends as a queue corpse. The editor gets `"Rendered MP4 is N MB — above the 200 MB archive limit. Trim the clip shorter in Descript, then Retry."`
  - **anything else** (network blip, S3 hiccup) → rethrows, so graphile-worker still retries with backoff.

  The 200 MB cap is not arbitrary: `archiveRemoteToS3` buffers the whole asset via `arrayBuffer()` before uploading, so raising it trades directly against the Basic dyno's 512 MB (R14/R15). A long 1080p clip is the usual way to exceed it — publish resolution is hardcoded `1080p` in the task. Regression tests: `src/jobs/tasks/descript-publish-and-archive.integration.test.ts`.
...
- **Trigger:** **Fan-out per clippable format**, from two places:
  ...
  - **Auto (2026-06-09, narrow; extended 2026-06-29):** `transcribe-whisper` phase-2 success fires the same fan-out for (a) `source_recording` items uploaded via `+ Custom MP4 Upload` (`source_type='source_recording'` bypasses brand allowlist/status/recency gates) and (b) new Starter Story youtube_long originals — gates in `src/lib/services/clip-ideas-auto.ts` (brand allowlist `AUTO_CLIP_IDEAS_BRANDS` defaulting to `starter-story`, Published + original + youtube_long, ≤7-day recency `AUTO_CLIP_IDEAS_MAX_AGE_DAYS`). The global post-`transcribe-whisper` auto-enqueue removed 2026-05-03 stays removed for everything else; per-(pillar, format) idempotency makes the auto path safe to race with a manual click.
- **Files:** Section picker → `src/lib/clip-section-agent.ts` (gpt-4.1, format-agnostic) + `src/lib/services/clip-sections-detect.ts` (advisory-lock + persistence). Hook writer → `src/lib/clip-hook-agent.ts` (gpt-4.1, per-(section, format)). Orchestrator → `src/lib/services/clip-idea-generate.ts`. Shared anchor utils → `src/lib/clip-anchor-utils.ts` (used by both v1 legacy and v2 agents). Worker task → `src/jobs/tasks/generate-clip-ideas.ts`.
...
- **Outputs:** a new `contentDrafts` row via the demote-then-insert tx (current row → `is_current=false`, new row → `is_current=true`, same `production_item_id`, version+1). `generatedBy = "draft-algo:v1.7:gpt-4.1:v13"` — the prefix tags the algorithm version, the suffix carries the underlying agent model+prompt version (`PROMPT_VERSION` bumped to 13 for the v13 proximity rule + prior-pairs few-shot + required-field validation — see V1.8 rule below). `modelUsage` populated. **V1.1 media write still active:** if the agent's `media_action` is `attach_pillar_full_video` or `attach_pillar_poster` AND the item has zero existing `production_item_media` rows AND the action is fulfillable (pillar has the asset, platform rule accepts the kind), the algorithm inserts a media row pointing at the pillar's S3 key (byte-cost zero — same bucket reuse) inside the same tx, and mirrors `mediaS3Key/Bucket/ContentType/posterS3Key` on `productionItems` to keep legacy single-media columns consistent.
- **Downstream:** none.
- **Rules:**
  ...
  - **Agentic tool loop (V1.5, 2026-05-10):** `generateDraft` is no longer a single-shot Responses call with `tool_choice` forcing `propose_draft`. It's now a loop (max 5 iterations) with `tool_choice: "auto"` — the agent picks tool calls itself. Specialized tools run as separate calls and return their results to the main agent via `function_call_output`; the main agent calls `propose_draft` as its terminal action. Architecture supports adding more tools (e.g. `find_money_quote`, `find_image_moment`) as one-file changes inside the same loop. Loop bounds: typical drafts run 1–2 iterations (zero or one tool call + the propose_draft); the 5-iteration cap throws so runaway tool-loops are visible in Sentry. Token usage on `content_drafts.modelUsage` accumulates across iterations.
  - **First specialized tool: `find_interesting_timestamps` (V1.5).** Lives in `src/lib/services/draft-algorithm/timestamp-finder.ts`. Registered in the agent's `tools[]` only when `substrate.kind === "transcript"` (cross-posts using source-body substrate skip the tool entirely — single-shot exactly like v1.4). The tool runs a separate gpt-4.1 sub-agent with the raw `transcripts.segments` jsonb array (not just the rendered markdown) and returns `Array<{ mmss, label, reason }>` with N validated picks. Each `mmss` is post-validated against the segments — out-of-range values are dropped with a `console.warn` so the main agent never sees fabricated timestamps. The tool is a small extra cost when triggered. We use gpt-4.1 deliberately for editorial judgment ("what's interesting" needs nuance).
...
| OpenAI (gpt-4.1) | `generate-clip-ideas` | The Splice clip-idea algorithm |
| OpenAI (gpt-4.1) | draft-gen | Per-platform draft copywriting |
