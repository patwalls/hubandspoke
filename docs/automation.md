...
USER / API ENTRY POINTS
  ...
  POST /api/clip-ideas/[id]/editor/export                 → clip-render (queue `media-heavy`, jobKey clip-render:<editId>) + draft-algorithm-run (first export only)  [UI: in-app clip editor "Export clip" — flag `clipEditor`, NO Descript involved]
