...
- **Downstream:** **narrow auto-enqueue of `generate-clip-ideas` (2026-06-09):** after a phase-2 transcript save, `maybeAutoEnqueueClipIdeas` fans out one `generate-clip-ideas` job per clippable format — but ONLY when every cost gate in `src/lib/services/clip-ideas-auto.ts` passes: brand in the auto allowlist (default `starter-story`, override `AUTO_CLIP_IDEAS_BRANDS`, empty string = kill switch), `status='Published'`, `source_type='original'`, `post_type='youtube_long'`, and published within the last 7 days (`AUTO_CLIP_IDEAS_MAX_AGE_DAYS`). The recency cap exists so whisper *backfill* scripts over old videos never fan out into LLM spend. Best-effort: enqueue failures log and never fail the transcript save (a retry would skip at the transcript-exists gate and lose the enqueue anyway). Everything else remains manual-only — the global post-transcribe auto-enqueue removed 2026-05-03 stays removed.
- **Rules:**
...
- **Trigger:** **Fan-out per clippable format**, from two places:
  ...
  - **Auto (2026-06-09, narrow):** `transcribe-whisper` phase-2 success fires the same fan-out for **new Starter Story long-form only** — gates in `src/lib/services/clip-ideas-auto.ts` (brand allowlist `AUTO_CLIP_IDEAS_BRANDS` defaulting to `starter-story`, Published + original + youtube_long, ≤7-day recency `AUTO_CLIP_IDEAS_MAX_AGE_DAYS`). The global post-`transcribe-whisper` auto-enqueue removed 2026-05-03 over Sonnet spend stays removed for everything else; the per-(pillar, format) idempotency below makes the auto path safe to race with a manual click.
...
- **Outputs:**
  ...
  - **Hook prepend (2026-06-25):** if `clip_ideas.hook_segments` is non-empty (the editor ticked "Include intro at top" in the triage dialog), phase 1 assembles `[hook ranges…, body]` instead of a single trim. Each entry is a source `[startSec,endSec]` range pulled from elsewhere in the video (typically the opening line). The assembly is one `ffmpeg -filter_complex` pass — per-segment `trim`/`atrim` + `setpts`/`asetpts`, then the `concat` filter — built by `buildConcatFfmpegArgs` in `src/lib/clip-assembly.ts` (pure, unit-tested in `clip-assembly.test.ts`). Same libx264/AAC/+faststart codecs and clean-PTS-from-zero property as the single trim, so Descript's importer accepts it identically. Invalid ranges are filtered (`isValidSegment`); empty/absent `hook_segments` is the unchanged single-range path. Descript still re-transcribes the assembled file, so on-screen captions cover the prepended hook for free. **Only the precise-cut paths honor `hook_segments`** — the full-video/agent paths can't prepend a non-contiguous range, so the triage UI disables them when an intro is active.
