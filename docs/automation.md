...
- **Rules:**
  ...
  - **Rollout gate:** `TRIGGER_ROUTING_CUTOFF_ISO` env var (ISO timestamp). When set, the sweep only processes pillars with `published_at >= cutoff` — prevents retroactively triggering the full historical catalog on first deployment. Remove via `heroku config:unset TRIGGER_ROUTING_CUTOFF_ISO` after the rollout window; no code deploy needed. Pillars with `published_at IS NULL` are excluded when the cutoff is active (treated as historic).
