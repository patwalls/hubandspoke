...
- **Outputs:** writes `productionItems.clicks` (sum of the post's link click counts) + `productionItems.leads` (max of `leads_count`, which the Rails side computes by matching `lead_conversions.content` to the link's `utm_campaign`). Only rows whose values changed are updated. Note: the Rails API also returns `hubspot_leads_count` (the HubSpot-form subset of `leads_count`), but this job does NOT persist it — the HubSpot-vs-native breakdown is read live by the detail-page LEADS hover via `/api/production-items/[id]/cta-link`, never cached in a column.
