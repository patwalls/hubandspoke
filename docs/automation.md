...
- **The AI brief (`brief.ts`):** Opus 5 (adaptive thinking), one pinned tool `write_playbook_brief`, forced. Inputs: the source transcript (speaker-labelled when available), the format Skill, the brand's best past posts of the format (headline + caption + views, as the style anchor — same idea as `derivative-hook.ts`), and an optional editor instruction ("Regenerate" in the AI bar). Output is CONTENT only (stat, headline + exact highlight phrases, footer, notes title, 3–5 phases, caption); `playbook-template.ts` owns the design. Measured: ~17 s, ≈ 10.6k in / 1.4k out tokens (≈ $0.09) per draft.
- **Template only for `Instagram PLAYBOOK`** (`src/lib/design-editor/templates.ts`); other formats keep the classic dialog. Adding a format = a brief tool schema + a template function + a row in `DESIGN_TEMPLATES`.
