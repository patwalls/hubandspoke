...
- **Rules:**
  ...
  - **Descript branch (V1.7, 2026-05-14):** after the caption-write tx commits, the algorithm checks `formats.is_clip_descript_format`. When true, it runs `runDescriptStepForDerivative` (in `src/lib/services/draft-algorithm/descript-step.ts`) which: (a) extracts the format Skill's `### Descript Clip & Pack Info` section verbatim — no `{{hook}}`/`{{startTimestamp}}` substitution because there's no clip-idea row backing this; the Skill is self-contained (e.g. "clip out the intro and only the intro"); (b) requires the *pillar* to already have a `descriptProjectId` — invokes Descript's `/jobs/agent` against that project with the Skill prompt; (c) stamps `descriptProjectId/Url` on the derivative and clears `descriptCompositionId` + publish state so the UI shows "rendering…" until the resolver fires; (d) inserts `repurposeTriggers` (`descriptImportPath="agent"`) and enqueues `descript-clip-resolve` which writes the new composition id back and auto-chains `descript-publish-and-archive`. **Skip reasons** surface as `descriptStep` on the return (and the manual `/draft` route's response): `skipped_no_skill` (no Descript section in the format Skill), `skipped_pillar_not_in_descript` (pillar has media but isn't in Descript yet — the four "Create in Descript" clip-promotion flows are still the only on-ramp for warming a pillar), `skipped_no_video_source` (text-only pillar), `skipped_already_done` (derivative already has a composition AND `force=false`). **Redraft (`force=true`)** re-fires Underlord and replaces the stale composition. **Errors don't fail the draft** — caption still saves; the Descript step's exception is logged with prefix `draft-algorithm v1.6 item=<id> descript_step error:` and dropped.
