...
CRON ENTRIES (src/jobs/crontab.ts, UTC)
  ...
  */20  youtube-download-sweep    → fan-out → youtube-download → transcribe-whisper → diarize-transcript (speaker detection; ≥2-min Whisper transcripts, own serialized queue)
...
USER / API ENTRY POINTS
  ...
  GET  /api/clip-ideas/[id]/editor                        → draft-algorithm-run (jobKey draft-algorithm-run:<itemId>; only when the idea's queue item has no written caption)  [UI: opening the in-app clip editor — flag `clipEditor`. The Post tab shows/edits the result; see post-draft.ts]
...
- **Files:**
  ...
  - `src/lib/descript.ts` — `publishDescriptComposition({projectId, compositionId, resolution, accessLevel})` returns `{jobId}`. `DESCRIPT_EXPORT_URL_PREFIX` constant identifies Descript's GCS export bucket for source-URL matching. **Dual-account routing (2026-08-10):** `authHeader(account?)` selects `DESCRIPT_API_TOKEN_HUBSPOT` when `account === 'hubspot'`, otherwise falls back to `DESCRIPT_API_TOKEN` (Pat's legacy account). All outbound call functions (`publishDescriptComposition`, `fetchDescriptJob`, `invokeDescriptAgent`, `duplicateDescriptComposition`, `cutSegmentWithRules`, `createDescriptProjectFromUrl`) accept `account?: string | null` and pass it to `authHeader`. Every call site reads `descript_account` from the item's DB row and threads it through. Existing rows with `descript_account IS NULL` use the legacy token with no behavior change. **All NEW projects go to `'hubspot'`** (`NEW_DESCRIPT_PROJECT_ACCOUNT` in `src/lib/services/descript-account.ts`) as of 2026-09-03: clip-queue promotions stamp it and carry it through the `clip-idea-precise-cut` payload (`descriptAccountFromPayload` — an explicit `null` in a payload means the legacy account and must NOT be `??`-defaulted away; only an absent field falls back to `'hubspot'`), and the draft-algorithm Descript step (`draft-algorithm/descript-step.ts`) — unmigrated in the 2026-08-10 change and briefly per-actor on 2026-09-03 (v837–v839) — now cold-imports pillars into `'hubspot'` and stamps both rows, while its warm path follows the pillar's existing stamp. The manual create-project route, cross-post re-aspect cold imports, and `coldImportPillar` hardcode `'hubspot'` as before. **Layout-pack registry (2026-09-03):** formats reference their Underlord layout pack via `formats.descript_layout_pack_id` → `descript_layout_packs` (name + Descript template-project id + page URL; rows validated at registration by `probeDescriptLayoutPack` in `src/lib/descript.ts`). `injectLayoutPackInstruction` (`src/lib/format-skill.ts`) splices a canonical apply-by-id instruction into the Skill's Descript section at every prompt-composition site: `loadPromotedClipFormat` (promote/reprocess/precise-cut service), the `clip-idea-precise-cut` layout phase, and the draft-algorithm Descript branch in `run.ts` (scoped so caption prompts never see it). The instruction includes the hard-won "query_layout_packs may return an empty list — do NOT stop there; apply by id" clause: Underlord's pack-listing tool is blind inside API-created projects, but apply-by-id works. One-shot backfill `scripts/backfill-descript-layout-packs.mjs` registered "Reels Layout", pointed the three starter-story clip formats at it, and cleaned pack prose out of all 12 Skills.
  ...
  - `src/lib/services/enrichment/shared.ts` — `archiveRemoteToS3` now handles files up to 500 MB. Any file with a `Content-Length` ≤500 MB is streamed directly to S3 via `Readable.fromWeb(res.body)` + `putObjectFromStream` (no RAM spike). Files >500 MB: rejected with a hard error. Servers that omit `Content-Length` fall through to the `arrayBuffer()` buffer path, capped at 200 MB. (2026-09-19: buffering every sub-200 MB file held a Descript render in RAM and pinned the 512 MB worker at R14 — 529M/103% for an hour+ — so streaming no longer waits for the 200 MB mark.) (2026-08-25: Descript was rendering some 1080p compositions at ~242 MB, silently poisoning the `media-heavy` graphile-worker queue on retries.)
...
- **Gating** (`createClipIdeaInDescript*` + clip-triage UI): the styling paths (Precise Cut, Buffered Cut + Layout Pack, Full Video) require a Skill/pack — service throws `FormatMissingDescriptPackError` (caught by the routes → 400) when missing, and the UI disables those items + shows "No Descript pack attached". **Plain Buffered Cut is exempt:** it runs no Underlord call, so it works for formats with no pack (e.g. X Quotables). Enforced by `loadPromotedClipFormat({ requireSkill: args.applyLayoutPack })` — only required when the pack will actually be applied.
- **Four promotion options surfaced in BOTH the clip-triage dropdown** (`src/components/dashboard/clip-triage-dialog.tsx`) **and the content-detail "Send to Descript" submenu** (`src/components/dashboard/content-detail.tsx` → `POST /api/production-items/[id]/send-to-descript?mode=…` → `reprocess-in-descript.ts`). Both surfaces show identical labels, descriptions, and order:
  ...
  | Buffered Cut + Layout Pack | `createClipIdeaInDescriptPreciseCut({ buffered: true, applyLayoutPack: true })` / `mode=buffered-pack` | New project with Claude's range ±60s padding, then the format layout pack applied via Underlord with `preserveAllFootage: true` so the buffer is never stripped. Disabled in the UI when no pack is attached. |
  | Buffered Cut (No Underlord) | `createClipIdeaInDescriptPreciseCut({ buffered: true })` / `mode=buffered` | New project with Claude's range ±60s padding. Plain trim — **NO Underlord call**, imports as-is, no layout pack. Works for formats with no pack (e.g. X Quotables). Editor finalizes boundaries and styles manually. |

  **Service-layer routing (2026-09-03):** `reprocess-in-descript` derives two flags: `isBuffered = mode === "buffered" || mode === "buffered-pack"` (adds ±60s); `applyPack = mode === "precise" || mode === "buffered-pack"` (fires Underlord). Plain `buffered` never calls Underlord regardless of any route param.
...
- **Layout step NEVER re-clips (2026-08-27):** when `applyLayoutPack=true` (Precise Cut, Buffered Cut + Layout Pack, or the agent-fallthrough), the `clip-idea-precise-cut` task always calls `buildLayoutPackPrompt({ preserveAllFootage: true })` — the hard no-trim override. This applies to both precise and buffered-pack modes: the composition is already the exact footage the editor wants (precise trim or deliberate buffer), so Underlord must only style it. The override's framing follows the format's resolved `aspectRatio` (16:9 formats like X Quotables stay horizontal). Plain Buffered Cut never reaches this phase (`applyLayoutPack=false`).
...
- **Punctuation on words (2026-09-20):** Whisper's word timestamps come back stripped of punctuation while its segments are punctuated; the pipeline now aligns the segment text back onto the timed words before saving (`src/lib/transcripts/punctuate-words.ts`, pure/tested: exact one-to-one match, else LCS over letters-only keys — timing untouched, unmatched words left as they were). Every consumer of `transcripts.words` (clip captions, design captions, sentence snapping, the transcript editors) sees sentence ends. Backfill for older rows: `npx tsx scripts/backfill-punctuate-words.ts [--apply]` (no API calls; ran on prod 2026-09-20).

### `diarize-transcript` — speaker detection for Whisper transcripts (2026-09-17)
- **Trigger:** auto-enqueued by `transcribe-whisper` right after a phase-2 transcript save (`maybeEnqueueDiarize(id, { force: true })` — new words carry no labels); manually by `POST /api/production-items/[id]/transcript/speakers` (the "Detect speakers" / "Re-detect" button in the transcript dialog, which bypasses the duration gate); in bulk by `scripts/backfill-diarize.mjs`. **Not feature-flagged.** Kill switch: `DIARIZE_LIVE=false` (same convention as `WHISPER_TRANSCRIBE_LIVE`).
- **Gate** (`src/lib/services/diarization/enqueue.ts`): Whisper transcripts only (caption-scrape rows have no audio of ours and no word timings), with extracted audio in S3, **≥ 2 min** long (short-form is overwhelmingly one voice), not already diarized unless forced.
- **Files:** `src/jobs/tasks/diarize-transcript.ts` (worker-only: ffmpeg + OpenAI SDK), pure logic in `src/lib/diarization/` (`assign.ts` words⇄turns, `identity.ts` cross-chunk speaker identity, `finalize.ts` labelled transcript, `types.ts`), `src/lib/services/diarization/name-speakers.ts` (Haiku naming), `src/lib/services/diarization/enqueue.ts` (gate; safe to import from web).
- **Why a second model instead of replacing Whisper:** `gpt-4o-transcribe-diarize` knows WHO is speaking but returns only coarse segments — **no word timestamps**, and it accepts **no vocabulary prompt**, so its text is worse than Whisper's on names. Whisper's words + timings stay canonical (clip cutting, captions and anchor matching all depend on them); the diarizer contributes speaker turns only, joined to words **by time**.
- **Inputs:** `{ productionItemId, runId?, force?, queue? }`. Reuses the 10-min mono 16 kHz opus chunks Whisper already left in S3 (`transcripts.audio_chunks`) — no re-download or re-extract of the video. (The API's docs don't list ogg; it accepts our ogg/opus chunks — verified.)
- **Outputs:** `transcripts.words[].speakerId`, `transcripts.segments[].speakerId` + `.speaker` (the DISPLAY name — this is the field `getTranscriptForPrompt` prints as `[MM:SS] speaker: text` and the transcript dialog renders, so every LLM prompt that reads a transcript gets speakers for free), `transcripts.speakers` (`TranscriptSpeaker[]`: id, name, nameSource `llm|user|null`, role `host|guest|narrator|unknown`, talkTimeSec, wordCount, firstSec), `transcripts.diarization` (job state + raw turns + token usage), `transcripts.diarized_at`. Migration `drizzle/0105_spicy_garia.sql` (additive).
- **How it works:**
  1. **Kickoff** (no `runId`): writes a fresh `runId` + `status: "running"` into `transcripts.diarization`, re-enqueues itself.
  2. **One chunk per invocation.** A diarize call on a 10-min chunk takes **~4–5 minutes** (measured), so a podcast in one invocation would be an hour-long job that any deploy restarts from zero. Each invocation diarizes one chunk, checkpoints, re-enqueues; a dyno restart costs at most the chunk in flight.
  3. **Cross-chunk identity.** The diarizer names speakers afresh per request (A, B…). From each chunk we cut a 2–8 s reference clip from the *middle of each speaker's longest turn* (`pickReferences`; ends trimmed — that's where crosstalk lives; a speaker with no ≥2 s turn gets NO reference, since a bad one is worse than none) and send them to later chunks as `known_speaker_names[]` = our stable ids (`S1`, `S2`…) + `known_speaker_references[]` (wav data URLs, API max 4 — we keep the 4 biggest talkers and upgrade a short early reference when a longer one appears). A label that comes back that we didn't send is a NEW speaker, even if it's spelled "A".
  4. **Finalize:** `labelTranscript` votes each Whisper word onto the turn it overlaps most, fills gaps from neighbours, and **absorbs islands** (≤2 words and ≤0.9 s sandwiched inside another speaker's speech — diarizer jitter and zero-length junk turns like `[A] 24.9–24.9 "Where"` are real and observed). A Whisper segment whose words change speaker mid-way is **split** at the change when both sides are ≥3 words and its text tokens line up 1:1 with its words (so the punctuated text is cut at the same word); otherwise majority wins. **One voice → no labels at all** (the single speaker is recorded in `speakers`, but "Speaker 1:" on every line of a monologue is noise).
  5. **Naming** (≥2 speakers): Haiku, one pinned tool, `temperature: 0`, fail-soft. Evidence = title, description, channel + owner, each speaker's talk share and first lines, and the opening with ids inline. A name is only APPLIED at `confidence: "high"` (a wrong name gets pasted into prompts as fact; "Speaker 2" doesn't), roles are kept at any confidence, and if two voices get the same name neither is trusted. Naming failing is not a task failure.
- **Rules:**
  - **Run isolation by `runId`:** every checkpoint and the final write are conditional `UPDATE … WHERE diarization->>'runId' = <mine>`. Starting a new run (re-detect, re-transcription) retires the old one at its next write — two runs can never interleave their turns.
  - **A user's rename is sticky.** `PATCH …/transcript/speakers` sets `nameSource: "user"`; re-detection carries it over by id and the LLM never overwrites it. The whisper upsert clears `diarized_at` (so detection re-runs) but deliberately keeps `speakers` — ids are assigned by first appearance, so on the same audio the rename still lands on the right voice.
  - jobKey `diarize:<itemId>` (`replace`); queues **`diarize`** (live) and **`diarize-backfill`** — both serialized, separate from each other and from `media-heavy`, so a backfill never delays a fresh video. The queue name rides in the payload because graphile's `Job` type doesn't expose it.
  - Errors: stamped into `diarization.error` and rethrown for graphile's backoff (`maxAttempts: 5`); only the last attempt flips `status: "failed"`. OpenAI SDK retries are OFF (`maxRetries: 0`) — graphile owns retries, and a silent SDK retry of a 5-minute call would double the bill.
- **Cost (measured 2026-09-17, real two-person interview):** $2.50/M audio-in + $10/M out ⇒ **≈ $0.22 per 10 min ≈ $1.35 per hour of audio**; ≈ 28 min of API time per hour of audio. Token usage per run is recorded in `diarization.usage`. The back catalogue at that date was ~680 h (≈ $900, ≈ 13 days on one queue) — which is why backfill is a **targeted, opt-in script**, not automatic: `node scripts/backfill-diarize.mjs --brand … --since … --limit …` (dry-run by default, prints the cost estimate).
- **Worker-slot cost:** the worker runs `concurrency: 2` (`src/jobs/worker.ts`) and a diarize call is a ~5-minute network wait that holds one slot the whole time (no CPU, ~no memory — but a slot). One new video = one slot busy for ~7 min per 15 min of audio, which is fine. A large backfill keeps a slot busy for HOURS, halving throughput for everything else: run backfills off-hours with a small `--limit`, or off the worker entirely on a one-off dyno (`heroku run:detached --app hubandspoke -- npx tsx scripts/diarize-run.ts <itemId>`). Raising worker concurrency would fix this properly but changes the memory/connection budget for every task — a deliberate decision, not made here.
- **Known gap:** `generate-clip-ideas` is auto-enqueued at the same moment and does NOT wait for speakers, so a brand-new video's first clip-idea pass reads an unlabelled transcript; re-generating later picks the labels up. Making clip-idea generation wait is a deliberate follow-up, not an oversight.
- **Local dev:** `npx tsx --env-file=.env.local scripts/diarize-run.ts <productionItemId> [--force]` drives the task through kickoff + every chunk in-process (no worker — see the warning under `clip-render`).
- **Diagnostics:**
  ```sql
  SELECT production_item_id, diarization->>'status' AS status,
         diarization->>'chunksDone' AS done, diarization->>'chunksTotal' AS total,
         jsonb_array_length(coalesce(speakers,'[]')) AS speakers,
         diarization->'usage' AS usage, left(diarization->>'error', 120) AS error
  FROM transcripts WHERE diarization IS NOT NULL ORDER BY updated_at DESC LIMIT 20;
  ```
...
- **Downstream:** none.
- **Rules:**
  - **THE CLIP in the prompt (`PROMPT_VERSION` 15, 2026-09-21):** for clip-idea-spawned items (`sourceClipIdeaId` set) the editorial block now carries (a) **what the clip actually says** — `loadClipCut` (`src/lib/services/clip-editor/post-draft.ts`): the kept words of the idea's `clip_edits` doc when the in-app editor has one (removals dropped, trims kept, sections in play order — `src/lib/clip-editor/clip-cut.ts`, pure), else every word inside the idea's `[startSec, endSec]`; rendered with the pillar range and the post-cut length — and (b) **`clip_ideas.extras`** (e.g. X Quotables' three verbatim `quotables`, which before only ever reached the Descript prompt). The system prompt rule: verbatim pulls come from THE CLIP word-for-word, never from elsewhere in the transcript; pre-picked extras win where they survived the cut. Before v15 the agent only saw the whole pillar transcript + a one-line anchor quote and had no idea which 25 seconds made the video.
  - **Drafted when the clip editor opens (2026-09-21):** `GET /api/clip-ideas/[id]/editor` → `ensureClipPostDraft` enqueues this task for the idea's queue item (every idea has one from generation, status `Idea`) unless its current draft already has a non-blank caption — so the editor's Post tab has copy while the cut is still being made, and export no longer waits on it. Keyed `draft-algorithm-run:<itemId>` (default `replace` mode: repeat opens fold into the queued run; a mid-flight run is re-run once afterwards and hits `already_filled`; a permanently failed run is retried). The Post tab's **Redraft** is `POST /api/production-items/[id]/draft` with `force+userInitiated`, after flushing the clip's autosave so the run reads the current cut.
...
### `clip-render` — in-app clip editor export (ffmpeg, no Descript) (2026-09-17, flag `clipEditor`)
- **Status:** behind the per-user `clipEditor` feature flag (`src/lib/feature-flags.ts` — Pat only). Nothing enqueues this task for anyone else.
- **Trigger:** `POST /api/clip-ideas/[id]/editor/export` → `exportClipEdit` (`src/lib/services/clip-editor/export.ts`) inserts a `clip_renders` row and enqueues `{ renderId }`. Also re-fired by the "Retry render" / "Edit clip & re-export" actions on the content-detail `ClipRenderStatusPill` (same route).
- **Files:** `src/jobs/tasks/clip-render.ts`; pure render pipeline in `src/lib/clip-editor/` (`doc.ts` model → `plan.ts` → `scene.ts`/`layout.ts` → `ass.ts` + `ffmpeg-args.ts`); `src/lib/services/clip-editor/install-rendered-media.ts`.
- **Inputs:** `{ renderId }`. The task reads ONLY the render row's `doc` snapshot (never the live `clip_edits.doc`), the source item's `media_s3_key`, and the source transcript's words (for captions).
- **Outputs:** MP4 at `<prefix>/<itemId>/<uuid>-clip-editor-render.mp4`; a `production_item_media` row at **index 0** with `source_url = clip-editor://render/<renderId>` (same "rendered clip owns slot 0" contract as `descript-publish-and-archive`, so the simulator, the legacy `media_s3_key` mirror and the publish/schedule flows pick it up unchanged); `clip_renders` → `done` with `duration_sec`, `render_seconds`, `output_*`; `tool_action` events (`tool=clip-editor`, `render_started` / `clip_rendered`) + `media_added`/`media_removed` content changes under `source: { kind: "tool", tool: "clip-editor" }`.
- **Workflow side effects live in the export service, not the task** — and mirror `createClipIdeaInDescriptPreciseCut` exactly so a clip made here is indistinguishable downstream: queue item → `Assigned` + `editorUserId` + title/hook/contentBody; idea → `assigned` + `acceptedProductionItemId`; an activity comment (deliberately terser than the Descript paths' — range, played duration and hook only, `buildClipCreatedComment`); fire-and-forget `draft-algorithm-run` as a safety net (the post is normally already written — it is drafted when the editor opens, see `draft-algorithm-run` → "Drafted when the clip editor opens"). No `repurpose_triggers` row and no `descript_*` columns are written, so `DescriptStatusPill` stays hidden for these items. **Re-export** (idea already `assigned` *by this editor*, i.e. it has a prior `clip_renders` row) queues a new render and skips the one-time steps; an idea decided any other way (killed, promoted via Descript) is refused with 409.
- **How the render works:** one ffmpeg pass. One input-seeked open of the source per doc section (`-ss`/`-t` before `-i`, read straight from a presigned S3 URL by HTTP range — the source is **not** downloaded; on failure the task falls back to download-then-render). Cuts are `select`/`aselect` on **frame number** after `fps=30` and `asetnsamples=1600` (1600 samples @ 48 kHz = exactly one video frame), so a clip with dozens of filler-word cuts is a single predicate, and audio and video are cut at the identical instant — sync cannot drift (verified with a beep/timecode source: onsets land frame-exact). Then the video is placed on the canvas using **exact pixel geometry from `resolveVideoBox` (`video-box.ts`) — the same function the browser stage uses** — which needs the source's display size: the task probes it by parsing the `ffmpeg -i` banner (`parseSourceDimensions`; there is no ffprobe in `@ffmpeg-installer`; rotation metadata is honored). Fit = scale+pad, Fill = scale+crop; **rounded corners** = an `alphamerge` mask whose rounded-rect distance field is evaluated by `geq` for ONE frame and looped (per-frame `geq` would be ~100× slower than the encode), composited with `overlay` onto a `color` background. **Both generated streams are `trim`med to the plan's exact frame count — an unbounded looped mask makes ffmpeg encode forever (it did, in development).** If the probe fails the graph falls back to aspect expressions and ignores inset/rounding. Verified on a dyno with prod's 2018 ffmpeg: rounded path 244 MB peak. Finally hook + captions are burned in with the `ass` filter (libass) using the fonts in `public/fonts/clip-editor/`. Text layout (line breaks, baselines) is computed once in `layout.ts` and both the browser preview and the ASS script draw the same pre-broken lines — that is what keeps preview == export.
- **Rules:**
  - **Single long invocation, deliberately not a self-re-enqueue chain** — an encode can't be checkpointed. A deploy's SIGTERM reaches ffmpeg (same process group) → non-zero exit → throw → graphile retries on the new dyno from the top (`maxAttempts: 3`).
  - **Every failure path stamps `status='failed'` + `error` before rethrowing**, and `heartbeat_at` ticks with ffmpeg progress so a SIGKILLed render (no exception to catch) reads as **`stalled`** after 5 min instead of "rendering" forever (`toRenderStatus` in `render-status.ts` — derived, never stored). This is the lesson from the stranded Descript publish `78b7cc20` above.
  - jobKey `clip-render:<clipEditId>` (`replace`) → at most one pending render per edit; `queueName: "media-heavy"` serializes it behind precise-cut / Descript publish work. A newer export marks older `queued`/`rendering` rows `superseded`, and the task re-checks "am I still the newest render for this edit?" on pickup.
  - Idempotent: `done`/`superseded` rows return early; `installRenderedClipMedia` finds its own `source_url` and no-ops on retry. Re-export replaces only rows with the `clip-editor://render/` prefix — manual uploads and Descript renders on the item survive — and **renumbers** the remaining media to 1…n (a blind +1 shift would leave a gap that grows with every export).
  - **Thread caps are load-bearing** (`ffmpeg-args.ts`): a dyno reports 8 cores, so by default the decoders and x264 each allocate 8 threads of 1080p frame buffers. Measured on a hubandspoke dyno with prod's ffmpeg, 40s clip: default **457 MB** peak RSS / 14s; decode 1 + filter 1 + x264 2 → **229 MB** / 15s. The caps stay until renders move off the 512 MB worker.
  - Prod's ffmpeg is the 2018 johnvansickle static build from `@ffmpeg-installer` — it has `ass`, `select`/`aselect`, `asetnsamples` (checked on a dyno), but predates variable-font support: **static TTFs only** in `public/fonts/clip-editor/`.
  - **Image layers (logos, 2026-09-21):** each visible `image` layer in the doc is one extra `-i` input after the video inputs (a bundled `/watermarks/…` asset is copied from `public/`, an S3 or URL source is downloaded first — never an https input), then `format=rgba,scale=<width>:-1` (height follows the picture), `colorchannelmixer=aa=` for opacity, and an `overlay` at the layer's anchor using `overlay_w/overlay_h` expressions, in z-order between the placed video and the ASS burn-in. A still's single frame repeats for the whole clip (`overlay`'s default eof_action). Verified locally 2026-09-21 (PNG with alpha at 85%); prod's 2018 build has these filters.
- **Cost:** no third-party API. ~2× realtime on a Basic dyno (40s clip ≈ 15–20s). The only LLM call in the feature is the optional on-demand "Remove filler words" button (`detect-fillers.ts`, Haiku, `temperature: 0`, one pinned tool returning word positions) — it runs on the web dyno inside the request, not in this task.
- **Local dev:** do NOT run `npm run worker` against a `/pulldb` database to test this — it would start executing every queued prod job and cron. Use `npx tsx --env-file=.env.local scripts/clip-editor-run-render.ts latest` (runs exactly one render in-process; needs S3 credentials in the env), and `npx tsx scripts/clip-editor-render-smoke.ts` for a no-DB/no-S3 pipeline check.
- **Diagnostics:**
  ```sql
  -- What's rendering / stuck / failing?
  SELECT id, status, progress, left(error, 120) AS error, render_seconds,
...
- **Trigger:** `requestAutoFrames(sourceItemId)` (`src/lib/services/design-editor/frames.ts`) from `findOrCreateDesignItem` (the Repurposed-queue click) and every design session load — jobKey `design-frames:auto:<sourceId>` (`preserve_run_at`), no-op once auto frames exist; `{ force: true }` (the editor's "try again") re-runs it. `requestFrameAt` (the editor's "Use this frame" scrubber, `POST /api/production-items/[id]/design/frames { atSec }`) enqueues `design-frames:at:<sourceId>:<sec>`.
- **Files:** `src/jobs/tasks/design-frames.ts`, `src/lib/services/design-editor/frames.ts`, `src/lib/design-editor/frame-times.ts` (pure: where to sample), `src/lib/services/ffmpeg-process.ts` (download + spawn/probe), `buildFrameGrabArgs` in `src/lib/design-editor/design-ffmpeg.ts`.
- **Keyed by the SOURCE video** (`design_frames.production_item_id` = the pillar; migration 0109 remapped earlier rows), so every post made from a video shares its library and its picks. The design item's frames route resolves the source itself.
- **What it does:** downloads the source to the dyno (see the ffmpeg note below), probes duration (falls back to the transcript's last word), picks 12 timestamps — **where the guest is talking** when the transcript is diarized (`frameTimes`: the founder's ≥8s stretches of speech, marks spread along them, inset from the cuts; even spacing otherwise), grabs each scaled to 1080 wide → S3 `<prefix>/<sourceId>/<uuid>-design-frame-<sec>.jpg` → one `design_frames` row per frame (`pending` → `done|failed`). Then **Haiku 4.5 ranks the 3 best founder shots** (forced tool `rank_cover_frames`; prefers the guest over the host, face sharp/lit, no burned-in graphics) → `rank` 1..3, `is_pick` = rank 1 (the cover); fails soft to middle frames. Measured locally: 12 frames + ranking in ~6s after the download.
- **ffmpeg on the dyno reads LOCAL FILES ONLY (2026-09-19):** the worker's static ffmpeg (2018 build) segfaults on any `-i https://…` — verified on a one-off dyno, SIGSEGV in ~150 ms with or without `-reconnect` — while local builds range-read https fine, which is how it went unnoticed (the first prod `design-frames` failed "couldn't read the source video's duration"; `clip-render` survived only via its download fallback). `downloadToFile` in `ffmpeg-process.ts` is used by `design-frames` and by `design-render` for video pages (one download per distinct source per render). A 130 MB source downloads in seconds from S3 on the dyno.
- **What it never does:** touch the design document. The editor polls `GET …/design/frames` while `pending` and puts the pick into the cover's photo slot itself (`applyPhotoPick`); a session load does the same server-side. **Fallback (2026-09-20):** when nothing more is coming and there is no pick (the job failed, or the video isn't archived), the cover gets the **YouTube thumbnail** (from `youtube_id`/`youtube_url`, `youtubeThumbnailFor` in `assets.ts`) — text baked in, but a real photo beats an empty slide. While the filmstrip is still being made the slot stays empty for a few seconds instead.
...
- **Trigger:** `POST /api/production-items/[id]/design/export` → `exportDesign` (`src/lib/services/design-editor/export.ts`) inserts a `design_renders` row (doc snapshot) and enqueues `{ renderId }`; also the "Retry render" / "Edit design & re-export" actions on the content-detail `DesignRenderStatusPill`.
- **Files:** `src/jobs/tasks/design-render.ts`; pure model/layout/render tree in `src/lib/design-editor/` (`doc.ts`, `layout.ts`, `render-tree.ts`, `playbook-template.ts`); services in `src/lib/services/design-editor/` (`brief.ts` AI, `session.ts`, `assets.ts`, `export.ts`, `install-design-media.ts`, `create-item.ts`, `render-status.ts`).
- **Video slides (2026-09-18):** a page holding a `video` element (a clip of the source, `startSec`–`endSec`) exports as an mp4 instead of a PNG: the page is baked as two stills — everything under the clip in z-order on the page background, everything over it transparent (`videoPageLayers`) — then ffmpeg (`design-ffmpeg.ts`, pure and unit-tested) input-seeks the source over https, scales/crops it by the same `coverGeometry` the stage uses (pan + zoom from `crop`), rounds the corners with the clip editor's one-frame looped `geq` mask, stacks under → clip → over, burns the `captions` element in as ASS (`captions.ts` cuts the transcript words inside the clip into cues; `design-ass.ts` places every pre-broken line with `\pos`), and encodes 1080×1080 30fps x264 + AAC with the clip editor's thread caps. A poster JPEG (frame 0 of the output) rides along as `poster_s3_key`. Progress weights video pages 10× image pages. Measured locally: two ~45s clips in 50s total; expect 1–2 min on the dyno.
- **What it renders (image pages):** each page's `DesignPage` → satori element tree (absolutely positioned single-line text nodes from the shared layout engine — the browser stage draws the identical markup, so preview == export) → SVG → **resvg** → 1080×1080 PNG → S3 (`<prefix>/<itemId>/<uuid>-design-page-N.png`) → `installDesignMedia`: pages take `production_item_media` indexes 0…n-1 with `source_url = design-editor://render/<renderId>`, other media renumbered after them, a previous design render replaced, manual uploads kept; legacy cover columns mirrored; `media_added`/`media_removed` under `source: { kind: "tool", tool: "design-editor" }`. No browser, no Chrome: satori (Vercel's HTML/CSS→SVG) + `@resvg/resvg-js` prebuilt native binary. ~1–2 s per page.
- **Images:** the task fetches every image element itself and hands satori data URLs — `asset` sources read from `public/` (the Starter Story · HubSpot Media wordmark in `public/watermarks/`), `s3` sources are presigned, `url` fetched. **Non-image or >25 MB responses are rejected** — `production_items.poster_s3_key` can be the VIDEO key (the legacy mirror falls back to it), which is exactly what tripped the first real render.
- **Workflow side effects (in the export service):** an `Idea` item → `Assigned` + editor + an `accepted` activity event (what the classic Accept does); the AI caption is written into the item's current content draft only if the caption is empty — and if no draft exists yet (the Draft Algorithm runs later on the worker) export seeds version 1 itself rather than racing it; `tool_action` events `render_started` / `design_rendered`. Re-export supersedes any queued/rendering row. jobKey `design-render:<designDocId>` (`replace`), queue `media-heavy`, `maxAttempts: 3`. Same failure contract as `clip-render` (stamp before rethrow; heartbeat → `stalled`).
- **Entry from the Repurposed (SPOKE) queue:** rows there are pillar × format candidates, not items. `POST /api/design/from-candidate` (`findOrCreateDesignItem`) creates the derivative item the classic "Assign editor" would — same fields, `createdVia: "design-editor"`, editor = the opener, `repurpose_triggers` dedup marker, `draft-algorithm-run` enqueued — **deliberately without `canva-create-copy`** (this editor replaces it), and reuses that item on later opens until it's Published/Killed.
- **The AI fill (`fill-brief.ts`, 2026-09-19 — replaces the playbook-only brief):** the format's TEMPLATE (`formats.design_template`, else the preset in `templates.ts`) is an ordinary DesignDoc whose elements carry slots; `listSlots` turns the AI slots into a prompt (name, hint, the sample text as the style example, maxChars from the box, allowed highlight colours) and Opus 5 (adaptive thinking, one pinned tool `fill_design`, forced) returns a value per slot + the caption. Inputs are otherwise unchanged: transcript with speakers, format Skill, the brand's best past posts, an optional instruction. `coerceFill` validates against the slots; `fillTemplate` (pure) substitutes, fills the system slots (photo pick / video title / channel from `accounts`), drops unfilled AI slots and clipless video pages, and reflows stacks. Adding a format = save a template on its format page (or add a preset + a `DEFAULT_PRESET_FOR_FORMAT` row).
- **Keyword in the caption prompt (2026-09-21):** `generateDesignFill` gets the item's attached keyword (`dmKeyword`) and the prompt's DM KEYWORD block makes the caption's CTA "comment <KEYWORD>" rather than "link in bio"; changing the keyword later rewrites the caption client-side (`dm-keyword-text.ts`).
- **Caption at open (2026-09-21):** `loadDesignEditorSession` calls `seedCaption(item, brief.caption)` every time the editor opens — the brief's caption becomes the item's content draft (v1, `generatedBy: design-editor:brief`) unless a caption already exists — so the editor's Post tab has copy from the start; export still calls it as a safety net. `findOrCreateDesignItem`'s `draft-algorithm-run` enqueue stays; whichever writes first wins and the other fills nothing.
- **Optional pages (2026-09-21):** `fillTemplate` drops a page whose AI text slots ALL came back empty — how the Full Story (up to 9 beats) and What My Apps (up to 5 apps) carousels get their length from the transcript. Pages without AI text (closers, CTAs) always stay. Presets now cover every Starter Story carousel format (`templates.ts` → `DEFAULT_PRESET_FOR_FORMAT`: PLAYBOOK, Tech Stack, Full Story, TMZ, What My Apps); `scripts/design-preview.ts` rasterizes a preset to PNGs for checking against real posts.
- **DM keyword on first draft (2026-09-19):** when the template has a `dmKeyword` slot, `loadDesignEditorSession` calls `ensureDmKeyword(itemId)` (`src/lib/services/dm-keyword.ts`) before drafting — the same go→go chain as `POST /api/production-items/[id]/dm-keyword` (which now delegates to `attachDmKeyword`), with the keyword chosen like the attach dialog's list order and the destination from `suggestCtaDestination`. Fails soft (logged) when the pool/destination is unavailable. Side effects live on the Rails short-links API: the keyword link is repointed and a `cta-<8 hex>` per-post link is minted/updated.
- **Clip look (video formats):** `formats.clip_template` is the style half of a ClipEditDoc (`clipLookSchema`); `loadOrCreateEdit` in `services/clip-editor/session.ts` applies it (`applyLook`) to every NEW edit of the format — existing edits are untouched.
- **Local dev:** `npx tsx --env-file=.env.local scripts/design-run-render.ts latest` runs one render in-process; `scripts/design-render-smoke.ts` renders the template from a hardcoded brief with no DB/S3/AI.
