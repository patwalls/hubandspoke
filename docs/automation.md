...
### `draft-algorithm-run` — The Draft Algorithm V1.2 (2026-05-09)
...
- **Outputs:** a new `contentDrafts` row via the demote-then-insert tx (current row → `is_current=false`, new row → `is_current=true`, same `production_item_id`, version+1). `generatedBy = "draft-algo:v1.2:claude-opus-4-7:v5"` — the `draft-algo:v1.2` prefix tags the algorithm version, the suffix carries the underlying agent model+prompt version (`PROMPT_VERSION` bumped to 5 for the new exemplar block layout + STRUCTURE RULE). `modelUsage` populated. **V1.1 media write still active:** if the agent's `media_action` is `attach_pillar_full_video` or `attach_pillar_poster` AND the item has zero existing `production_item_media` rows AND the action is fulfillable (pillar has the asset, platform rule accepts the kind), the algorithm inserts a media row pointing at the pillar's S3 key (byte-cost zero — same bucket reuse) inside the same tx, and mirrors `mediaS3Key/Bucket/ContentType/posterS3Key` on `productionItems` to keep legacy single-media columns consistent.
- **Downstream:** none.
- **Rules:**
