...
- **Files:**
  ...
  - `src/lib/descript.ts` — `publishDescriptComposition({projectId, compositionId, resolution, accessLevel})` returns `{jobId}`. `DESCRIPT_EXPORT_URL_PREFIX` constant identifies Descript's GCS export bucket for source-URL matching. **Dual-account routing (2026-08-10):** `authHeader(account?)` selects `DESCRIPT_API_TOKEN_HUBSPOT` when `account === 'hubspot'`, otherwise falls back to `DESCRIPT_API_TOKEN` (Pat's legacy account). All outbound call functions (`publishDescriptComposition`, `fetchDescriptJob`, `invokeDescriptAgent`, `duplicateDescriptComposition`, `cutSegmentWithRules`, `createDescriptProjectFromUrl`) accept `account?: string | null` and pass it to `authHeader`. Every call site reads `descript_account` from the item's DB row and threads it through. Existing rows with `descript_account IS NULL` use the legacy token with no behavior change. **Which account NEW projects land in varies by entry point:** clip-queue promotions (Precise/Buffered Cut via `promote-clip-idea` → `clip-idea-precise-cut`) resolve it from the promoting user via `resolveDescriptAccountForActor` in `src/lib/services/descript-account.ts` (Pat → legacy account, since his personal Descript login isn't a member of the HubSpot workspace; everyone else → `'hubspot'`), and the resolved value rides the task payload + gets stamped on the row. The manual create-project route, cross-post re-aspect cold imports, and `coldImportPillar` still hardcode `'hubspot'`. The draft-algorithm Descript step (`draft-algorithm/descript-step.ts`) passes no account at all — it was never migrated in the 2026-08-10 change, so repurpose-tab derivatives cold-import into the legacy account for every user.
