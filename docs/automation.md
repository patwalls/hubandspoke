...
- **Rules:**
  ...
  - **Per-request network budget (`SC_TIMEOUT_MS = 30s`, 2026-08-22):** every fetcher in `sc-fetchers.ts` passes `AbortSignal.timeout(SC_TIMEOUT_MS)`. The worker dyno has no router timeout, so before this a stalled SC connection hung the awaiting job indefinitely — on 2026-08-22 the 19:00 and 20:00 UTC runs both never returned (no `sync_logs` row, no worker log line), and at concurrency=2 the second hang wedged the entire worker dyno: heartbeat stale 17min, 206 jobs queued, oldest due job unpicked for 16min. Recovery was `heroku ps:restart worker`. Healthy runs take 52-452s, so a 30s per-request budget sits well above the slow tail. An aborted request surfaces as a normal per-item error (`TimeoutError`), caught by the `syncPerformanceData` loop, stamped via `stampSyncResult` and counted as 1 credit like any other failed item.
