...
### `design-frames` — cover-photo frames from the source video (2026-09-18, flag `designEditor`)
- **Status:** behind the per-user `designEditor` flag (Pat only).
- **Trigger:** `requestAutoFrames` (`src/lib/services/design-editor/frames.ts`) from `findOrCreateDesignItem` (the Repurposed-queue click) and from every design session load — jobKey `design-frames:auto:<itemId>` (`preserve_run_at`), no-op once rows exist. `requestFrameAt` (the editor's "Use this frame" scrubber, `POST /api/production-items/[id]/design/frames { atSec }`) enqueues `design-frames:at:<itemId>:<sec>`.
- **Files:** `src/jobs/tasks/design-frames.ts`, `src/lib/services/design-editor/frames.ts`, `src/lib/services/ffmpeg-process.ts` (shared spawn/probe), `buildFrameGrabArgs` in `src/lib/design-editor/design-ffmpeg.ts`.
- **What it does:** presigns the SOURCE item's `media_s3_key`, probes duration from the `ffmpeg -i` banner, and grabs 12 evenly spaced frames between 4% and 96% (`-ss` input seek = ranged https reads, ~1s each) scaled to 1080 wide → S3 `<prefix>/<itemId>/<uuid>-design-frame-<sec>.jpg` → one `design_frames` row per frame (`pending` → `done|failed`, `(item, sec)` unique). Then **Haiku 4.5 looks at the filmstrip** (12 base64 JPEGs, forced tool `pick_cover_frame`) and marks the best founder shot `is_pick` — face sharp, no burned-in graphics, room for the headline; fails soft to the middle frame. `atSec` mode grabs a single frame. Measured locally: 12 frames + pick in ~13s.
- **What it never does:** touch the design document. The editor polls `GET …/design/frames` while `pending` (rows pending, or the auto job queued, or the filmstrip done but not yet picked) and inserts the pick as the cover's full-bleed `Photo` itself (`ensureCoverPhoto`); a session load does the same server-side when no editor is open. The first draft deliberately leaves the cover EMPTY rather than falling back to the platform thumbnail (usually a YouTube thumbnail with its own text baked in).
- **Queue:** `media-heavy` (serial with clip/design renders), `maxAttempts: 2`. Sources with no archived video log and return (the editor still offers uploads + the source's pictures).
