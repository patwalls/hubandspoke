...
- **Outputs:** N rows in `clip_ideas` (default 10 per pillar) and N paired `production_items` rows (`source_type='clip'`, `source_clip_idea_id` back-link, `pillar_content_item_id` → source pillar, `status='Idea'`, `post_type='instagram_reel'`, `format='Reel: Repackage Section w/ Hook'` — canonical from `PROMOTED_CLIP_FORMAT` in `src/lib/services/promote-clip-idea.ts`; was `'Repackage section with hook'` until 2026-05-05, when 4,400+ rows got rewritten via `scripts/backfill-promoted-clip-format.mjs` and the wrong-named Pillar format was deleted). `clip_ideas.accepted_production_item_id` is wired to the new prod_item row (note: confusingly named — set at *generation*, not on triage acceptance).
...
### `draft-algorithm-run` — The Draft Algorithm V1.1 (2026-05-08)
...
- **Outputs:** a new `contentDrafts` row via the demote-then-insert tx (current row → `is_current=false`, new row → `is_current=true`, same `production_item_id`, version+1). `generatedBy = "draft-algo:v1.1:claude-opus-4-7:v3"` — the `draft-algo:v1.1` prefix tags the algorithm version, the suffix carries the underlying agent model+prompt version. `modelUsage` populated. **V1.1 also writes media:** if the agent's `media_action` is `attach_pillar_full_video` or `attach_pillar_poster` AND the item has zero existing `production_item_media` rows AND the action is fulfillable (pillar has the asset, platform rule accepts the kind), the algorithm inserts a media row pointing at the pillar's S3 key (byte-cost zero — same bucket reuse) inside the same tx, and mirrors `mediaS3Key/Bucket/ContentType/posterS3Key` on `productionItems` to keep legacy single-media columns consistent.
- **Downstream:** none.
- **Rules:**
