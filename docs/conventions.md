...
- New slot kinds go in `SLOT_KINDS` (doc.ts) + `fillElement` (template-fill.ts) + the inspector's `SLOT_OPTIONS`. Prefer a system slot filled from the item over asking the AI for something we already know. Picture slots: `photo` = the filmstrip's cover pick (every photo slot shows the same founder shot); `frame` = a different still per slot, in library order (`storyFrames`: runner-up shots first, then the filmstrip by time, the cover pick left out) — for story slides that each want their own picture. Both fall back to the placeholder until the frames land, and `applyPhotoPick` fills them then.
- **Optional pages:** a template page with at least one AI text slot whose AI text ALL comes back empty is dropped from the post (`fillTemplate`). That is how a carousel gets a variable length — build the template with the MAXIMUM number of pages (9 story beats, 5 apps) and say in the later slots' hints "Leave EMPTY if…". A page with no AI text (a static closer, a CTA) is always kept.
- **Every design is a square** (`IG_SQUARE`, 1080×1080) — Pat's rule (2026-09-21), even for formats whose published posts were 4:5. The canvas is stored per document and nothing downstream assumes a square (`reflowStacks` and the renderers read `doc.canvas`), so other sizes are a preset change away when he wants them — but don't ship one.
