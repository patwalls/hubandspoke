...
| Cross-post (different platform, manual) | Active | `POST /api/production-items/[id]/cross-post` | `productionItems` (sourceType=`cross_post`) | Manual companion to the auto cross-post-scan. See `docs/post-classification.md` for the platform-based axis vs. repost |
| Cross-post (manual suggestions v2) | Active | "Populate queue" button on `/[brand]/queue` Cross-post tab → `POST /api/cross-post-scan`; per-post `capture-velocity-snapshot` jobs at publish+15m/30m/1h/2h/4h keep the `view_snapshots` cohort fresh | `view_snapshots`, `cross_post_decisions`, `productionItems.crossPostConfidence` | Velocity-gated + LLM-recommended; scanner NOT on cron — operator clicks the button when they want up to 10 new ideas. Accept (→ `Assigned`) or kill on each row via the triage dialog; required reasons train future runs. |
