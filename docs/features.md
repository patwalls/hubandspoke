...
| Cross-post (different platform, manual) | Active | `POST /api/production-items/[id]/cross-post` (per-target Cross-post submenu on `/content/[id]`; `assign:true` + optional `editorUserId` from the v3 queue modal) | `productionItems` (sourceType=`cross_post`) | See `docs/post-classification.md` for the platform-based axis vs. repost |
| Cross-post candidate queue (v3) | Active | `/(dashboard)/[brand]/queue` Cross-post tab → `GET /api/cross-post-queue`; click a row → `CrossPostTriageDialog` → per-target `POST /api/production-items/[id]/cross-post` (creates + assigns); "Not interested" → `POST /api/production-items/[id]/cross-post-dismiss` | `productionItems` (live filter on Published originals from last 7d), `contentEvents` (`type='cross_post_dismissed'` for the 30d hide-list) | Replaces v2's LLM-recommended (source × target) Idea rows with a live percentile-within-format candidate list — top 25% of `views` over a 90-day same-format cohort (≥10 posts; small cohorts skip). No populate step, no cron — query runs on every page load against `productionItems.views` (kept fresh by `performance-decay`). Operator picks targets; the algorithm doesn't. |
...
| Overlay text (on-video burn-in) | Active | `vision-extract-sweep` cron (every :55) → per-item `vision-extract`; dispatcher's Haiku-with-poster call also writes here when it picks `source='overlay'` | `productionItems.overlay` | Dedicated column for verbatim burn-in text on the cover/video. **Source of truth = OCR on the poster image** (Haiku vision); title is NOT a reliable proxy. `hookSource='vision'` ⇒ this clip's hook came from a designed overlay. Recovery scripts after the failed title-trust experiment: `scripts/revert-repackage-overlay-backfill.mjs`, `scripts/enqueue-vision-for-repackage.mjs`. |
