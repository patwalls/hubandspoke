...
| Drafts (versioned per-platform copy) | Active | `GET\|POST\|PATCH /api/production-items/[id]/drafts`, `POST .../drafts/generate`, `POST .../generate-caption` (IG-only, 2026-05-07) | `contentDrafts` | AI gen via Anthropic Opus 4.7 (kept on Anthropic — copywriting nuance is load-bearing); UI auto-persists on blur. **IG caption auto-fill (2026-05-07):** repost into an Instagram post type → after the seed tx commits the route enqueues `generate-instagram-caption`, which loads (a) the pillar transcript, (b) `formats.instructions`, (c) up to 8 most-recent published `contentBody` rows for the same `(brand, format, postType)` as exemplars, and writes a new draft via `draft-agent` (now accepting `pastCaptions[]`). The Regenerate button on the simulator caption panel calls `POST /api/production-items/[id]/generate-caption` synchronously. Idempotency: skips when current draft caption is non-empty AND `generatedBy !== "copy:source"`, unless `force=true`. Prompt caching (`cache_control: ephemeral`) on system + format-stable preamble + past-captions block. The `/drafts/generate` endpoint also passes `pastCaptions` for IG post types so manual draft regeneration gets the same context. |
