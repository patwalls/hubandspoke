...
| Overlay text (on-video burn-in) | Active | Dispatcher short-circuit + `PUT /api/production-items` title-edit sync; vision sweep also writes here | `productionItems.overlay` | Dedicated column for verbatim burn-in text painted onto the cover/video. For "Reel: Repackage Section w/ Hook" the editor types the overlay into `title` and we mirror it; the unified dispatcher's `'overlay'` source maps here too. `hookSource='overlay'` ⇒ this clip's hook came from a designed overlay. Backfill: `scripts/backfill-repackage-hook.mjs`. |
