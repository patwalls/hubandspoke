...
| Repost (same platform) | Active | `POST /api/production-items/[id]/repost` | `productionItems` (sourceType=`repost`, repostedFromItemId) | Body accepts optional `{ editorUserId, status }`: queue v2 sets `status='Ready To Publish'` so reposts skip the Idea/Assigned cycle, manual content-detail submenu defaults to `Idea` for back-compat. Writes a `repost_created` activity event when status is `Ready To Publish`. See `docs/post-classification.md` for the rules. |
