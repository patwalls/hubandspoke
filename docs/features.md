...
| Item detail (metadata, comments, drafts, transcript) | Active | `/(dashboard)/[brand]/content/[contentId]`, `GET\|PATCH\|DELETE /api/production-items/[id]` | `productionItems`, `contentComments`, `contentDrafts`, `transcripts`, `productionItemMedia` | **Pre-publish X drafting surface (2026-05-07):** when `isPrePublish && postType === "x"`, the Details tab swaps to a two-column layout — `ContentPreview` simulator on the LEFT (editable tweet text + photos), form metadata on the RIGHT, no Instructions panel, no separate Preview tab. Other platforms still use form-left / Instructions-right and keep the Preview tab. **Published link / Published date rows are hidden whenever `isPrePublish`** (any platform). **Preview tab disappears for published items**. **Manual media upload (X only, 2026-05-07):** drag-drop or click "Add photo or video" on the simulator card to add up to 4 photos OR 1 video to a pre-publish tweet. Routes through `POST /api/production-items/[id]/media/presign` → browser PUT to S3 → `POST /api/production-items/[id]/media`. Hover an existing slide → × to delete via `DELETE /api/production-items/[id]/media/[mediaId]`. Same `productionItemMedia` schema enrichment writes for published tweets, so reposts of published tweets pull their photos from this same table automatically. Refused on published items. |
...
| Repost (same platform) | Active | `POST /api/production-items/[id]/repost` | `productionItems` (sourceType=`repost`, repostedFromItemId), `productionItemMedia` (mirrored from source for `postType='x'`), `contentDrafts` (v1 row pre-filled with source tweet text, `generated_by='copy:source'`, for `postType='x'`) | Body accepts optional `{ editorUserId, status }`. **Queue parity (2026-05-06):** both creation paths — the manual `Actions → Repost` submenu on `/content/[id]` and the queue v2 triage dialog — now POST `status='Ready To Publish'` so reposts skip the Idea/Assigned cycle and emit a `repost_created` activity event. `status='Idea'` is still accepted for back-compat. **X seed (2026-05-06):** the route also mirrors source `productionItemMedia` rows onto the new repost and inserts a v1 `contentDrafts` row pre-filled with the source tweet text so the simulator card on the redirect target renders the original tweet + photos immediately, ready to edit. Author display in the simulator resolves from `accountId` via the joined account, so changing the account updates the card. **On-demand source enrichment (2026-05-07):** if the source's `enrichmentCompletedAt` is null, the route calls `enrichSingleItem` synchronously before building the new row so the seeded copy of `contentBody` + media is populated even when the auto-enrich sweep hasn't reached the source yet. Best-effort — failures are logged and the create proceeds. See `docs/post-classification.md` for the rules. |
